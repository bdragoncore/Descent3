...
#define GAMMA_SLIDER_UNITS 145 // BUGFIX (PiccuEngine #6): Allow gamma all the way down to 0.1

#define IDV_GAMMAAPPLY 5
#define IDV_AUTOGAMMA 6
#define IDV_CHANGE_RES_WINDOW 10
// IDV_MSAA_CYCLE / IDV_AF_CYCLE / IDV_VFOG_CYCLE are defined in config.h.
// BUGFIX #10: they must NOT collide with the menu option sheet IDs
// (IDV_VCONFIG=12 .. IDV_CCONFIG=17), or newuiMenu::DoUI() treats a button
// press as a page navigation (IDV_VFOG_CYCLE=13 == IDV_GCONFIG=13 jumped to
...
static void config_gamma() {
  ...
  slider_set.min_val.f = 0.1f; // BUGFIX (PiccuEngine #6): Allow gamma down to 0.1 for bright monitors
...
// VIDEO MENU
//
// MSAA sample count -> display label. 0 = off.
...
// Next MSAA level in the Off -> 2x -> 4x -> 8x cycle.
