...
// ── Cross-validation: dump intermediate state as JSON for Python 1:1 compare ──
...
extern float Window_w2;
extern float Window_h2;
extern vector Matrix_scale;

static std::string cv_fmt(float v) {
    char buf[64];
    snprintf(buf, sizeof(buf), "%.8g", v);
    return buf;
}

TEST_F(D3GameRenderTest, CrossValidateDumpState) {
    BeginFrame();
    ...
    const char* out_path = "build/tests/cross_validate.json";
#ifdef RENDER_OUTPUT_DIR
    // Write next to render output
    static char path_buf[512];
    snprintf(path_buf, sizeof(path_buf), "%s/cross_validate.json", RENDER_OUTPUT_DIR);
    out_path = path_buf;
#endif

    FILE* fp = fopen(out_path, "w");
    ASSERT_TRUE(fp) << "Cannot open " << out_path;

    fprintf(fp, "{\n");
    fprintf(fp, "  \"window_w2\": %s,\n", cv_fmt(Window_w2).c_str());
    fprintf(fp, "  \"window_h2\": %s,\n", cv_fmt(Window_h2).c_str());
    fprintf(fp, "  \"matrix_scale_x\": %s,\n", cv_fmt(Matrix_scale.x()).c_str());
    fprintf(fp, "  \"matrix_scale_y\": %s,\n", cv_fmt(Matrix_scale.y()).c_str());

    auto dump_mat = [&](const char* name, float mat[4][4]) {
        fprintf(fp, "  \"%s\": [\n", name);
        for (int r = 0; r < 4; r++) {
            fprintf(fp, "    [%s, %s, %s, %s]%s\n",
                cv_fmt(mat[r][0]).c_str(), cv_fmt(mat[r][1]).c_str(),
                cv_fmt(mat[r][2]).c_str(), cv_fmt(mat[r][3]).c_str(),
                r < 3 ? "," : "");
        }
        fprintf(fp, "  ],\n");
    };

    dump_mat("modelview", gTransformModelView);
    dump_mat("projection", gTransformProjection);
    dump_mat("viewport", gTransformViewPort);
    dump_mat("full_transform", gTransformFull);
    ...
    for (int i = 0; i < d3_render_test::CUBE_NUM_VERTS; i++) {
        ...
        world_pos.z() = z;
        float y = world_pos.y() * cp - world_pos.z() * sp;
        z = world_pos.y() * sp + world_pos.z() * cp;
        world_pos.y() = y;
        world_pos.z() = z;

        g3_RotatePoint(&cube_pts[i], &world_pos);
        g3_ProjectPoint(&cube_pts[i]);
        g3_CodePoint(&cube_pts[i]);

        fprintf(fp, "    {\n");
        fprintf(fp, "      \"world\": [%s, %s, %s],\n",
            cv_fmt(world_pos.x()).c_str(), cv_fmt(world_pos.y()).c_str(), cv_fmt(world_pos.z()).c_str());
        fprintf(fp, "      \"camera\": [%s, %s, %s],\n",
            cv_fmt(cube_pts[i].p3_x).c_str(), cv_fmt(cube_pts[i].p3_y).c_str(), cv_fmt(cube_pts[i].p3_z).c_str());
        fprintf(fp, "      \"screen\": [%s, %s],\n",
            cv_fmt(cube_pts[i].p3_sx).c_str(), cv_fmt(cube_pts[i].p3_sy).c_str());
        fprintf(fp, "      \"clip_code\": %d\n", cube_pts[i].p3_codes);
        fprintf(fp, "    }%s\n", i < d3_render_test::CUBE_NUM_VERTS - 1 ? "," : "");
    }
    fprintf(fp, "  ]\n");
    fprintf(fp, "}\n");
    fclose(fp);
    ...
    EndFrame();
    ReadPixels();
    SavePNG("CrossValDump");

    printf("Cross-validation state dumped to %s\n", out_path);
    SUCCEED() << "State dumped for Python cross-validation";
}
