...
static InMemoryFile* GetInMemoryFile(CFILE* cf) {
    auto* mcf = ToMock(cf);
    if (!mcf || !mcf->file) return nullptr;
...
int cfseek(CFILE* cfp, long int offset, int where) {
    ...
    if (new_pos > infile->data.size()) return -1;
...
void cf_Rewind(CFILE* cfp) {
    ...
    mcf->position = 0;
    auto* infile = GetInMemoryFile(cfp);
    if (infile) infile->position = 0;
}

int cfgetc(CFILE* cfp) {
    auto* mcf = ToMock(cfp);
    if (!mcf) return EOF;
    if (mcf->position >= mcf->size) return EOF;
    
    auto* infile = GetInMemoryFile(cfp);
    if (!infile || infile->data.empty()) return EOF;
...
int cf_WriteBytes(const uint8_t* buf, int count, CFILE* cfp) {
    ...
    if (!(mcf->flags & 0x2)) return 0;  // Not CFF_WRITING
